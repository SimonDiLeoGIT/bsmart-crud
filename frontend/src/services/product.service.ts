...
class ProductService {
  static async getProducts(page: number = 1, sortBy: string = 'id', sortOrder: string = 'asc', perPage: number = 15, name: string = ''): Promise<ProductListResponseInterface> {
    const response = await ApiService.makeRequest(`/products/${sortBy}/${sortOrder}?page=${page}&per_page=${perPage}${name !== '' ? `&name=${name}` : ''}`);
